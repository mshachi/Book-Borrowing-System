...
class addCustomerDialog(object):
    ...
    def get_customer_info(self):
        ...
        valid_id_path = self.openpicbutton.text() 
        # Assuming this holds the path to the selected image
        ...
        if self.contains_only_valid_characters(phone_number):
            phone_number_valid = phone_number
            # Prepare the data to be inserted into the database
            customer_info = (name, gender, phone_number_valid, valid_id_path)
        else:
    ...
    def contains_only_valid_characters(self,text):
        # Define a regular expression pattern to match only numbers, '-', '(', ')' and '+'
        pattern = r'^[0-9()+-]*$'

        # Use the re.match() function to search for the pattern in the text
        match = re.match(pattern, text)

        # If a match is found and it covers the entire string, return True
        if match and match.span()[1] == len(text):
            return True
        else:
            return False
