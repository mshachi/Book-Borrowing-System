...
class ReturnBookDialog(object):
    ...
    def confirm_return_book(self):
        try:
            ...
            return_date_str = self.return_date.text()  # Return date is taken from the button text
            ...
            rental_due_date_result = cursor.fetchone()
            if not rental_due_date_result:
                raise ValueError("Rental due date not found for the book.")
            rental_due_date_str = rental_due_date_result[0]

            rental_due_date = QDate.fromString(rental_due_date_str, QtCore.Qt.DateFormat.ISODate)
            return_date = QDate.fromString(return_date_str, QtCore.Qt.DateFormat.ISODate)
            ...
            # Inform the user of the successful return
            QMessageBox.information(None, "Return Book",
                                    f"Returned by {customer_name} on {return_date_str} with overdue fee {overdue_fee:.2f}")

            self.dialog.accept()

        except sqlite3.Error as e:
            QMessageBox.critical(None, "Database Error", f"Error returning book: {e}")
            if conn:
                conn.rollback()
            if conn:
                conn.close()
            ...
            QMessageBox.critical(None, "Input Error", f"Error: {ve}")
