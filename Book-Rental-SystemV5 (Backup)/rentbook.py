...
class RentBookDialog(object):
    ...
    def confirm_rent(self):   
        try:
            ...
            try:
                ...
                # Check if the book is reserved within the rental period
                if self.is_book_reserved(book_id, rent_date_str, due_date_str):
                    QMessageBox.warning(self.dialog, "Error", "Book is reserved within the selected rental period.")
                    return
    ...
    def is_book_reserved(self, book_id, start_date, end_date):
        conn = sqlite3.connect("library.db")
        c = conn.cursor()
        
        # Query to check if the book is reserved within the rental period
        query = """
            SELECT 1 FROM reserve
            WHERE BookID = ?
            AND ReservationDate BETWEEN ? AND ?
        """
        c.execute(query, (book_id, start_date, end_date))
        result = c.fetchone()
        
        conn.close()
        
        # Return True if there is a reservation within the rental period
